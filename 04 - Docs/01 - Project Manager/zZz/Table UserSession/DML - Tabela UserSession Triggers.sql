...
as begin
	...
	if not exists(select * from inserted)
		...
		begin
			...
			set @contents	= 'Delete do registo de userSession  nº ' + convert(varchar(10), (select userSessionId from deleted));
	...
	-- insert
		...
		begin
			...
			set @contents	= 'Insert do registo de userSession  nº ' + convert(varchar(10), (select userSessionId from inserted));
	...
	-- update
		...
		begin
			...
			set @contents	= 'Update do registo de userSession  nº ' + convert(varchar(10), (select userSessionId from inserted));
